            ...
            if showAddRow {
                VStack(alignment: .leading, spacing: 6) {
                    HStack(spacing: 8) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Bundle ID").font(.caption2).foregroundStyle(.secondary)
                    ...
                    HStack {
                        Button("Save") {
                            ...
                            manager.add(AppInsertionOverride(
                                bundleID: bid,
                                displayName: name.isEmpty ? bid : name,
                                mode: newMode
                            ))
                            showAddRow = false
                            newBundleID = ""; newDisplayName = ""; newMode = .clipboardPaste
                        }
                    ...
                    VStack(spacing: 0) {
                        ForEach(manager.overrides) { override in
                            HStack(spacing: 8) {
                                ...
                                Button(action: { manager.remove(id: override.id) }) {
