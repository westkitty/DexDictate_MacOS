...
public class CommandProcessor {
    ...
    public func process(_ text: String, customCommands: [CustomCommand] = []) -> (String, DictationCommand) {
        ...
        // 1. Custom hot-word commands: "Dex [keyword]"
    ...
    /// Returns processed text if the transcript matches "Dex [keyword]" for any custom command.
    /// Strips the hot-word portion and substitutes the command's `insertText`.
    private func processHotWordCommand(_ text: String, commands: [CustomCommand]) -> (String, DictationCommand)? {
        ...
        let spokenKeyword = String(text[keywordRange]).trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
