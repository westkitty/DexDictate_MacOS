...
private func runWonderPath() {
    ...
    check(path, engineSource.contains("applyLifecycle(.transcriptionCompleted"), "transcription completion still returns the engine to ready through the lifecycle model")
    check(path, engineSource.contains("outputCoordinator.deliver("), "engine routes output through explicit output coordination")
    check(path, engineSource.contains("vocabularyManager.applyEffective("), "engine applies effective layered vocabulary")
